...
class BaseSolver():
    ...
    def write_log(self, log_name, log_value):
        ...
        if type(log_value) is dict:
            log_value = {key: val for key, val in log_value.items() if (
                val is not None and not math.isnan(val))}
        if log_value is None:
            pass
        elif len(log_value) > 0:
            # ToDo : support all types of input
            if 'cmatrix' in log_name:
                self.log.add_figure(
                    log_name, log_value[0], global_step=self.step)
                ...
                self.log.add_embedding(
                    log_value[0], metadata=log_value[1], tag=log_name, global_step=self.step)
            elif 'wave' in log_name:
                signal, sr = log_value
                self.log.add_audio(log_name, torch.FloatTensor(
                    signal).unsqueeze(0), self.step, sr)
